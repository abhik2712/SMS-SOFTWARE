			...
			<div id="section">
				...
				<div class="content">
					...
					<?php
						if(isset($_POST["submit"]))
						{
							...
							{
								$sq="insert into student(RNO,NAME,FNAME,DOB,GEN,PHO,MAIL,ADDR,SCLASS,SSEC,SIMG,TID) values('{$_POST["rno"]}','{$_POST["name"]}','{$_POST["fname"]}','{$edate}','{$_POST["gen"]}','{$_POST["pho"]}','{$_POST["email"]}','{$_POST["addr"]}','{$_POST["cla"]}','{$_POST["sec"]}','{$target_file}','{$_SESSION["s_id"]}')";
				...
				<label> Parent's Mail Id</label><br>
					...
					<select name="cla" required class="input3">
						...
						<?php 
							 $sl="SELECT DISTINCT(c_name) FROM class";
							$r=$a->query($sl);
								if($r->num_rows>0)
									{
										...
										{
											echo "<option value='{$ro["c_name"]}'>{$ro["c_name"]}</option>";
										}
									}
						?>
					...
					<br><br>
						...
						<?php 
							 $sl="SELECT DISTINCT(c_sec) FROM class";
							$r=$a->query($sl);
								if($r->num_rows>0)
									{
										echo"<option value=''>Select</option>";
										while($ro=$r->fetch_assoc())
										{
											echo "<option value='{$ro["c_sec"]}'>{$ro["c_sec"]}</option>";
										}
									}
						?>
